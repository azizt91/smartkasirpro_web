...
<div class="bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div class="flex items-center pb-6 border-b-2 border-gray-200 mb-8">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900">💇‍♂️ Tambah {{ \App\Models\Setting::getStoreSettings()->employee_label ?? 'Pegawai' }} Jasa Baru</h1>
                <p class="text-gray-600 mt-1">Buat data mekanik/terapis/kapster baru untuk perhitungan komisi.</p>
            </div>
        </div>
            ...
            <div class="p-8">
                <form action="{{ route('employees.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="flex items-center justify-end space-x-4 pt-4 border-t border-gray-200">
                        ...
                        <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm transition-colors duration-200">Simpan {{ \App\Models\Setting::getStoreSettings()->employee_label ?? 'Pegawai' }}</button>
                    </div>
                </form>
            </div>
        </div>
